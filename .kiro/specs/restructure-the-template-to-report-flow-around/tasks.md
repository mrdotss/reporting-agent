...
- [ ] 3. Phase 2 — sections
  ...
  - [ ] 3.5 The two new block compilers
    - **Also resolves the title-resolution gap task 3.4 found**, if scoped in: every shipped catalogue entry opens with a `heading` whose `text` `expand_sections` never supplies from the entry's `title_id`, so no catalogue-declared section compiles today. Fix belongs wherever a section's expansion turns `title_id` into heading text — either `expand_sections` resolving it via `messages.text(entry.title_id)` before emitting the heading's config, or this task's block compilers doing it at compile time. Whichever, it needs a decision on where, since `expand_sections` is pure and has no `Messages` today
    - `blank_rows_table` — section 13's author-filled table with ruled empty rows; `resource_table` cannot emit a row with no resource. Emits a `Table` of `EmptyCell` rows
    - `subscription_facts` — section 1's subscription-level facts, which belong to no resource. Emits a `Table` of `TextFactCell`
    - Each needs: an entry in `BLOCK_TYPES` in **both** halves inside the `BLOCK TYPES` sentinel region, an entry in `BLOCK_CONFIG` in both inside `BLOCK CONFIG`, an entry in `BLOCK_COMPILERS` — the import-time `assert set(BLOCK_COMPILERS) | {_ROW_TYPE} == set(BLOCK_TYPES)` fails a half-done addition — and a corpus fixture, since the corpus guard asserts every declared block type appears in it
    - Assert an `EmptyCell` row leaves the anchored pass with nothing to compare and ledger completeness unaffected. An empty cell is not a zero, and that is asserted rather than assumed
    - **Also fix `catalog/sections.v1.json`'s `resource_table` column config**, found the same way: every `resource_table` entry declares bare-string `columns` (e.g. `["type", "count"]`), but `read_column_entries` requires objects with a `kind` discriminator (`{"kind": "attribute", "attribute": "resource_type"}`). Twelve entries across the catalogue need correcting to real values from `COLUMN_ATTRIBUTES` — the catalogue must round-trip through the real compiler, not just through its own structural tests
