...
- [ ] 3. Phase 2 — sections
  ...
  - [ ] 3.5b `subscription_facts` — DEFERRED, decision defaulted after being asked twice with no response
    ...
    - **Asked the user via `ask_question` twice across two turns (2026-08-26, cycles 12–13); no response arrived either time.** Defaulted to "skip for now, move to 3.6" — the option that keeps the most doors open and matches the finding that nothing currently depends on it. Not a silent skip: recorded here with the full open question above, so whoever picks this up (human or agent) has the actual decision to make, not just a missing checkbox
  ...
  - [x] 3.6 The five-step wizard shell
    - `app/lib/profiles/wizard.ts` (moved from `app/lib/templates/wizard.ts`): `WizardStepId` is `"identity" | "sections" | "period" | "document" | "preview"`, `WIZARD_STEPS` five entries, `STEP_FOR_FIELD` maps `schema_version`/`provider`/`identity` → identity, `sections` → sections, `period` → period, `front_matter`/`design` → document — verified against the real `REQUIRED_TOP_LEVEL_KEYS[3]` in `definition.ts`, not assumed
    - `issuesByStep`, `canAdvance`, `canReturnTo`, `openingStep`, `previousStep`, `nextStep`, `stepById`, `hasAnyIssue` carried over unchanged in behavior. `completionProblems` still calls `collectDefinitionIssues(definition, {mode:"run"})` unmodified; `blockCount` becomes `sectionCount`, and `{kind:"no_blocks"}` becomes `{kind:"no_sections"}` — `validateSections` has no `mode` param and never rejects an empty array itself, so the wizard's own zero-count check is still what enforces it, mirroring the v1/v2 split exactly
    - `provider` collected on step 1; `PROVIDERS = ["azure","aws","onprem"]` already exists in `definition.ts` with only `azure` accepted by the validator — no separate UI selector exists yet (that lands with step 1's own component, a later task), so this task's scope is the model-level lock, not a picker
    - **Provider immutability, extracted as a pure function** (`checkProviderImmutable` in `lib/actions/templates.ts`, following `resolveDesignFromBrand`'s established pattern of keeping the DB-touching caller thin around pure logic): compares `existingVersion.definition.provider` against the incoming definition's, refusing a publish that changes it once any version exists. Enforced only when both sides declare a `provider` (v1/v2 definitions never trip it). **The delegated implementation had left this untested** — a real behavior with zero test coverage, caught only because I verified the diff directly rather than trusting the report; I extracted it to a pure function specifically so it could be tested without Postgres, added 5 tests in `test/provider-immutable-at-publish.test.ts`, and mutation-checked (inverted the equality check; 2 of 5 tests correctly failed, both restored to pass)
    - All 5 importers of the old path updated; `wizard.test.ts` moved to `lib/profiles/` (colocated, matching this repo's existing convention); old `lib/templates/wizard.ts` and its test deleted after confirming nothing else imports it. A stale entry in `test/boundaries.static.test.ts`'s `SPEC_PURE_MODULES` list (which swept `lib/templates/` and expected `wizard.ts` there) was found and fixed by the delegated agent — a real guard that would otherwise have failed
    - Suite: app 2976 passed / 247 skipped / 0 failed (was 2970 baseline), typecheck clean except the one pre-existing error, lint unchanged at 13 problems (1 error, 12 warnings) — all verified independently by me, not trusted from the delegated agent's report
    - **Defect found while starting 3.7, recorded here rather than silently patched into 3.6's history**: `wizard-shell.tsx`'s `renderStep` switch wires the new `"sections"` case to `<StepScope>` and the new `"document"` case to `<StepMetrics>` — both v1/v2 components reading `definition.scope`/`definition.metrics`, fields that do not exist on a v3 definition (`TemplateDefinition`'s type has no v3 variant at all; it is one fixed v1/v2 shape with `scope`/`metrics`/`blocks` always present). This typechecked only because `WizardShell`'s own `definition` state is cast `as TemplateDefinition` regardless of actual `schema_version`, and neither the delegated agent's diff review nor mine (focused on `checkProviderImmutable`, which I did verify directly) re-audited every line of `wizard-shell.tsx`'s switch statement. A v3 profile through step 2 today would read `undefined.resource_types` or worse. **Not a regression in already-shipped behavior** — no v3 profile has reached this screen yet, since 3.7+ is what builds the rest of the v3 UI — but it means step 2 was left in a broken intermediate state rather than a stubbed one. Fixed by 3.7 building the real `StepSections` component and wiring it in place of `StepScope`; `TemplateDefinition` itself remains v1/v2-only (a proper v1/v2/v3 union is a larger, separate type-system change touching 32 files and 156 usages, out of scope for either task)
    - _Requirements: 3.2, 3.6, 7.2, 7.3_

  - [x] 3.7 Step 2 — the section list and the inspector (first pass; full scope explicitly deferred, see below)
    - `components/templates/step-sections.tsx`, wired into `wizard-shell.tsx`'s `case "sections"` **in place of the wrongly-wired `<StepScope>`** the note under task 3.6 recorded. This is the actual fix for that defect, not a separate patch
    - `definition` treated as `unknown` throughout, read defensively for `sections`/`identity.language` — deliberately not `TemplateDefinition` (which has no v3 variant). `sectionCatalogue: readonly SectionCatalogueEntry[]` threaded as a plain prop from the server (`AZURE_SECTIONS` in `app/lib/profiles/sections.ts` is `import "server-only"` and cannot cross into a client component directly), resolved in `templates/[id]/edit/page.tsx` the same way `catalog`/`thumbnails` already are — no new API route needed
    - Left: authored sections grouped under Inventory/Utilisation/Closing by looking up each one's catalogue `group`. Fixed/always-position entries render a "Fixed" label with **no** reorder control; free entries get keyboard-focusable up/down move buttons with an `aria-live="polite"` region announcing the new position — **a deliberate first-pass simplification of the full drag-handle-plus-insertion-point design**, stated as such in the component's own comments, not a hidden shortcut
    - Right: a minimal inspector — resolved title (via `messageText`, confirmed client-importable: `lib/messages/catalog.ts` carries no `server-only` marker), group, position, metric-bearing, needed resource types, current presentation (read-only)
    - Basic add-section control respecting `repeatable: false` (an already-authored non-repeatable entry is not re-offered; a repeatable one always is)
    - **Explicitly deferred, not silently dropped**: resource chips drawn from a scan, the metric picker with presets and per-metric statistic selection, percentile-estimator label copying from the catalogue, and an editable presentation-choice control — all four need scan-resolved resource data or a metric-selection UI this pass does not build, and are real product-UI work for a later task
    - 7 tests (`step-sections.test.tsx`): grouping, fixed-position sections carry no reorder control, up/down reordering produces the correct resulting order (asserted on the actual array, not just that `onChange` fired), a move past a boundary is a no-op, `repeatable:false` correctly excludes an already-authored entry from the add list while a repeatable one stays offered, adding appends correctly. Mutation-checked the boundary guard myself (weakened the boundary condition from `||` to `&&`; the boundary test caught it — it surfaced as a render-time crash rather than a clean assertion failure, still a real catch, not a silent pass)
    - Suite: app 2985 passed / 247 skipped / 0 failed (was 2976), typecheck clean except the one pre-existing error, lint unchanged at 13 problems (1 error, 12 warnings) — all re-verified independently
    - _Requirements: 7.4, 7.5, 7.6, 7.7, 8.2, 8.3, 9.3, 9.4, 10.1, 10.2, 10.3, 10.4, 10.5, 10.6, 10.7, 10.8, 10.9 (partial — see deferred scope above)_

  - [x] 3.8 The emit estimator and its cross-language fixture
    - `app/lib/profiles/emit.ts`: `estimateEmit(section, scan, catalogue)` → `{headings, charts, tables, figures, matchesZeroResources}`, plus `estimateProfileEmit` for the profile-level rollup and `matchedResourceCount` as its own tested unit. Pure — no fetch, no db
    - **The formula is not "statistics × metrics × matched_resources" as originally scoped — corrected after reading the real compile arithmetic directly, not assumed.** `expand_sections` never reads a section's own `metrics` field at all (confirmed by reading the whole module: no reference to the key exists), so a section's authored metric choice has **zero effect on today's compiled output** — it is collected by the wizard for a future wiring task, not yet consulted. Figures instead scale with the **catalogue's own static `config.columns`** metric-kind count, which is what `read_column_entries` actually reads. Every shipped `resource_table`/`top_n_table` entry currently declares zero metric-kind columns (only `attribute`/`fact`), so **the real-catalogue estimate is currently always 0 figures** — a fact about the shipped catalogue the fixture asserts directly rather than hiding behind a plausible-looking non-zero number
    - Two more real arithmetic gaps found and fixed the same way, only surfaced because the Python mirror test actually compiles a real snapshot rather than asserting against the estimator's own logic: (1) a `per: "resource"` expansion emits one block **per matched resource**, uncapped by `MAX_TABLE_ROWS` — that cap bounds rows within one `per: "section"` table, not how many per-resource blocks a section expands to; (2) the compiler's `omitted_row` mints exactly **one additional `Figure`** (the truncated count itself) when a `per: "section"` table's matched set exceeds 500 rows. Both are now in the formula and in the shared fixture
    - Shared fixture at `agent/tests/fixtures/emit-estimate/cases.json` (11 cases), read by `app/lib/profiles/emit.test.ts` (vitest, against the real shipped `AZURE_SECTIONS`) and `agent/tests/test_emit_estimate_mirror.py` (pytest, which builds a real synthetic snapshot from the same resource counts and asserts the real `compile_document`'s `figure_count` matches) — following the established `scope-union/cases.json` precedent in this repo exactly, including its meta-tests proving the corpus isn't degenerate (both a zero and non-zero figure case, both matched and zero-resource cases, the 500-row cap actually exercised). 8 cases use real shipped catalogue entries; 3 use one hand-built synthetic entry (marked `"synthetic": true`) carrying a real metric-kind column, since no shipped entry has one yet to exercise the positive-figure arithmetic against
    - Case-insensitive resource-type matching (`matchedResourceCount`): the scan's `type_counts` keys are Resource Graph's own lower-case casing while the catalogue declares Azure's canonical casing — mirrors `azure/inventory.py#read_counts`'s own `.casefold()` rule, confirmed by reading that function rather than assumed
    - Mutation-checked: removing the `per:"resource"` multiplier fails exactly the two tests that name it
    - **Deferred, stated explicitly**: wiring the estimate into Step 5's UI (`step-preview.tsx` currently types `definition` as the v1/v2-only `TemplateDefinition` and receives neither a scan nor a catalogue prop — threading both through `WizardShell` is real additional surface work, the same shape of deferral already made in tasks 3.5/3.7). The inspector-level "counted from the scan before anything runs" messaging in Step 2 is also not wired to a live scan yet, since `StepSections` (task 3.7) does not currently receive one
    - Suite: agent 4670 passed / 0 failed (was 4656), app 3006 passed / 247 skipped / 0 failed (was 2985), typecheck clean except the one pre-existing error, lint unchanged at 13 problems — all run once from a clean state
    - _Requirements: 11.1, 11.2, 11.3, 11.4, 11.6, 11.7 (11.4's "stays selected" and "step 5 states" wiring deferred per above)_
  ...
  - [x] 3.10 Authored matches: the table and the write — schema, upsert, and guard done; **wiring into the publish path deferred, asked twice with no answer**
    - `app/lib/db/schema.ts`: `report_profile_authored_matches` (`template_version_id` → `report_template_versions` cascade, `scan_id` → `subscription_scans`, `section_id`, `matched_count`, `matched_resource_ids` jsonb, `unique(template_version_id, section_id)`). Migration generated (`0010_great_toad.sql`), additive only, applied and verified against a real Postgres 17
    - `app/lib/profiles/authored-matches-store.ts`: `writeAuthoredMatches(templateVersionId, scanId, matches)` — an upsert keyed on the unique pair, since `insertVersion` may return an **existing** version when the digest is unchanged, in which case the rows already exist and must still pick up the current scan. `readAuthoredMatches(templateVersionId)` for task 3.11's coverage appendix to read from later
    - **Kept out of the definition, verified structurally rather than by a new digest test**: `definitionSha256` (in `lib/templates/version.ts`) is computed from the definition object alone and has no parameter through which a scan id or a resource id could reach it — the "same digest for different scans" property the task asks to assert is therefore already guaranteed by the digest function's own signature, not something a new test needs to prove independently
    - **Not projected to the browser**: `test/authored-matches-projection.static.test.ts` asserts `lib/db/views.ts` never imports the table or references its columns — mutation-checked by adding a reference and confirming the guard fires, then reverting
    - 5 integration tests against a real Postgres (`test/db/authored-matches-store.integration.test.ts`): one row per section readable back; a second write for the same `(version, section)` updates in place rather than duplicating (task's own stated reason for the upsert, verified by actually racing it); an empty `matches` array writes nothing; rows for a different template version are untouched by a write to this one; a `scan_id` naming no real scan is refused by the database. Also fixed two now-stale hardcoded table/constraint lists in `test/migrations.static.test.ts` and `test/db/schema.integration.test.ts` that predated even the `brands`/`subscription_scans` tables, not just this one
    - **`writeAuthoredMatches` is not yet called from `publishTemplateVersion`.** Wiring it needs a `scanId` — which subscription scan the consultant was looking at while authoring the profile — and there is genuinely no mechanism anywhere that tracks one today: `report_templates` carries no subscription reference at all (a template is deliberately reusable across every connected customer), and neither `StepSections` (3.7) nor the emit estimator (3.8) receives a scan. **Asked as a design question twice this session with no answer** (does the wizard gain an explicit scan/subscription picker, or auto-select the most recently completed scan?) — recorded here rather than guessed at, since a wrong guess means rebuilding real wizard-state wiring, not adjusting a formula. Everything shipped here (schema, upsert semantics, projection guard) is independently correct regardless of how that question resolves — the only remaining step once it does is one call site
    - Suite: app 3260 passed / 0 failed / 0 skipped with a real Postgres enabled (was 3006 without one — every previously-skipped integration suite ran this time), 3008 passed / 252 skipped / 0 failed without one (was 2985/247) — typecheck clean except the one pre-existing error, lint unchanged at 13 problems
    - _Requirements: 9.5_

  - [x] 3.11 Drift in the coverage appendix
    - `compile/sections.py`: `AuthoredMatch` (recorded resource ids from a prior publish) and `compute_section_drift(definition, *, catalogue, view, authored_matches)` → `tuple[SectionDrift, ...]`. Pure — `authored_matches` arrives as a value the caller already read, the same "caller fetches, pure module folds" split `historical_selections`/`comparison` already follow. One `SectionDrift` per section that HAS a recorded match — a section never published before is skipped, not reported as "everything added" — and a section with **zero drift is still included** with two empty tuples, per Req 19.3's "never excluded silently": an omitted row would be indistinguishable from a section this appendix never checked at all
    - `compile_document` computes drift once (when both `catalogue` and `authored_matches` are supplied) and threads it into `BlockContext.section_drift`, typed `tuple[object, ...]` there specifically to avoid a circular import — narrowed at the one call site in `record.py` that reads it
    - `compile/blocks/record.py`'s `compile_gaps_and_coverage` renders one prose statement per `SectionDrift` after its existing gaps table — added/removed resources named explicitly, in mist neutrals (prose carries no color at all, so this is structural rather than a styling choice), never withheld
    - `DERIVED_COUNT_KINDS` extended with `scope_added_count`/`scope_removed_count`. `verify/derived_counts.py`'s `check_derived_counts` gained optional `view`/`catalogue`/`authored_matches` parameters (all default `None`, every existing caller unaffected) and re-derives both new kinds by recomputing the **full** `compute_section_drift` result and summing the matching kind — not by re-resolving one section in isolation, which would let the re-derivation silently depend on `compute_section_drift`'s own id-derivation scheme without either side's tests catching a drift between them
    - 4 new message ids (`doc.drift.added/removed/unchanged`) added to both language catalogs, verified against the mirror guard
    - `authored_matches` documented in `agent/AGENTCORE_INTEGRATION.md`'s `generate_report` payload, in this commit, with its exact "present exactly when" condition
    - **The static mirror guard is NOT extended to `authored_matches`, and the field is NOT added to `app/lib/aws/agentcore.ts`'s TS payload type — deliberately, not an oversight.** The existing guard (`mirror.static.test.ts`'s `generate_report` section) compares keys `report_pipeline.py::_resolve_run_facts` actually reads via `payload.get(...)` against the TS type; `authored_matches` is read by `compile_document`, not that function, and more fundamentally: nothing in `report_pipeline.py` reads `authored_matches` from the invoke payload yet, and nothing in `lib/runs/invoke.ts` sends it — both are downstream of task 3.10's still-unresolved scan-selection question. Adding it to the TS type now would document a field no code path ever sets; extending a guard with no real sender or reader on either side would guard a path that does not exist, exactly the situation `liftDefinition`'s own note (task 3.12) already states as the reason to skip a mirror for something the runtime never sees
    - Mutation-checked: swapping `added`/`removed` in `compute_section_drift` fails 2 of 5 drift tests by name; corrupting a derived-count value fails the re-derivation mismatch test by name
    - **Also deferred, for the same reason**: stating the scan the version was authored against and its resource-scanned/carrying-statistics/gap/changed-in-scope counts in `verification_record` — that is provenance about a **scan row** (`scan_id`, `resource_count`, `type_counts`, etc.), reachable only once a real `authored_matches` row names one, which is the same production gap task 3.10 recorded
    - Suite: agent 4679 passed / 0 failed (was 4675), app 3260 passed / 0 failed / 0 skipped with a real Postgres — typecheck and lint unchanged at baseline
    - _Requirements: 19.1, 19.2, 19.3, 19.4, 19.5, 19.6, 19.7, 22.1, 22.2 (the payload-mirror portion of 22.1/22.2, and the scan-provenance statement, deferred alongside task 3.10's own deferral)_

  - [x] 3.12 The migration lifter
    ...
    - `design` lifts into the returned `brand` values (a caller creates the actual Brand row; this module does not write). A v2 `front_matter` passes through with its own values winning per-key over v3's draft-mode defaults — verified against the real `accept-schema-version-2-front-matter.json` fixture, whose complete `front_matter` is carried unchanged since every key it declares overrides the default. A v1 definition (no `front_matter` at all) gets v3's draft-mode minimum rather than an empty object that would fail the very `mode: "draft"` bar this task's own test holds every fixture to
    - **The task's own "unmapped by design" list was incomplete — verified against the real catalogue, not assumed from the requirements text.** Reading `catalog/sections.v1.json`'s `expands_to` directly shows `cover`, `executive_summary`, `kpi_row`, `capacity_vs_usage`, `distribution_chart` and `appendix_methodology` never appear in ANY entry — not just the five named (`heading`, `rich_text`, `page_break`, `row`, `comparison_delta`). `cover` is the one exception with a real destination that is not a section: its only field, `subtitle`, lifts into `front_matter.cover.subtitle` directly (verified against a real v3 fixture). The other five — `executive_summary`, `kpi_row`, `capacity_vs_usage`, `distribution_chart`, `appendix_methodology` — genuinely have no v3 destination in the shipped catalogue and are reported unmapped, stated as data (`UNMAPPED_BLOCK_TYPES`) rather than silently under-reported
    - Mapping heuristic for everything else: a block maps onto the catalogue entry whose `expands_to` declares its block type AND whose `needs_resource_types` intersects the block's own effective scope (`scope_override`, or the template default) — computed from real catalogue data, not a hardcoded block-type table that could drift from it. Case-insensitive intersection, matching `azure/inventory.py#read_counts`'s own `.casefold()` rule
    - **Multiple blocks resolving to the same non-repeatable entry merge into one section — found by running the lifter against the real `accept-every-block-type.json` fixture, not designed in advance.** A v1 definition's separate `resource_table`/`top_n_table`/`timeseries_chart` blocks, all scoped to the same resource type, each independently resolve to `vm_utilization` — creating one section per block violated that entry's own `repeatable: false` the moment two blocks targeted it. Deduplicated by `(entry.key)` for non-repeatable entries; a repeatable entry never merges
    - An ambiguous match (more than one candidate entry's scope intersects) reports unmapped with `reason: "ambiguous_catalogue_destination"` rather than guessing — Req 20.4's "the wizard presents the report and requires the author to choose" is exactly what an ambiguous automatic choice would silently defeat
    - Writes nothing to `report_template_versions`; the draft is handed to the existing unvalidated `saveDraft` path
    - **Test corpus counts corrected**: the task's stated "44 fixtures at schema_version 1 and 10 at 2" does not match the real corpus (verified directly: 16 v1 + 3 v2 accept fixtures, 20 total including 1 v3 which needs no lifting). `lift.test.ts` reads the real manifest rather than a fixed count, so it tracks the corpus as it actually is rather than restating a number that was already wrong
    - For every accepting v1/v2 fixture, the produced draft is asserted accepted by `collectDefinitionIssues(draft, {mode: "draft"})` — required building v3's own draft-mode defaults (`identity.language`, complete `design`, `front_matter.cover/document_control/toc`) precisely, since no existing v3 `EMPTY_DRAFT` equivalent existed to copy
    - Mutation-checked: disabling non-repeatable deduplication fails the corpus test naming the exact `"not repeatable and already appears earlier"` validator message
  ...
  - [x] 3.14 The rename — routes, redirects, and the noun; the literal-copy guard extension deliberately deferred
    - UI routes moved: `/templates` → `/report-profiles`, `/templates/[id]/edit` → `/report-profiles/[id]/edit`. API routes moved: `/api/templates` → `/api/report-profiles`, `/api/templates/[id]` (+ `/preview`) and `/api/templates/catalog` → their `report-profiles` equivalents. Shapes unchanged
    - Redirects from every former route: the four UI/API paths each keep a stub at the old location — `redirect()` for the two UI pages (preserving the `id` segment), a 308 (permanent, method-preserving) for the four API routes, chosen specifically over 302/303/307 because this endpoint's `POST` (publish a version) must never silently downgrade to a `GET` on the hop. 6 new tests assert the exact status and rewritten `Location` for every method on every route, including query-string preservation
    - The noun **Report Profile** resolved through `messageText` wherever it was already the mechanism in force: 7 message values (both languages) carrying "template"/"Template" in their displayed text — `doc.front_matter.template_version`, `ui.template.untitled_placeholder`, `ui.run_form.template_label`, `ui.run_form.no_templates_hint`, `ui.run_form.no_template_versions_hint`, `ui.run_form.front_matter_hint`, `ui.run_form.period_explanation`. **The ids themselves keep the word "template" — internal symbols, not surfaced, per the task's own rule** — only the displayed `en`/`id` strings changed. Plus the sidebar nav label, the run-detail page's "Template" field label, the wizard's "Template name" field label, and the front-matter placeholder-help description — none of which were routed through `messageText` at all, fixed as raw literals matching each file's own existing convention
    - `report_templates`, `report_template_versions` and every `template_version_id`/`TemplateView`/`TemplateDefinition` internal symbol kept their names, unchanged — confirmed by grep across the whole app that no remaining reference is user-visible
    - **The literal-copy guard extension is deliberately NOT done, recorded here rather than silently declared complete.** `message-literals.static.test.ts` currently scans only `components/reports/` and `components/scan/`. Checked both `components/templates/` (the profile/section surface) and `components/brand/` directly: **both carry substantial pre-existing raw-literal copy that was never brought under this discipline**, unrelated to this rename — `app/(app)/templates/page.tsx` alone had 9+ untracked literals before this task touched it, and `components/templates/*.tsx` spans dozens of files most with double-digit raw-string counts. Extending the guard's scan directories today would either fail immediately on that pre-existing content, or require migrating all of it to `messageText` first — real, substantial work with its own scope, not a rename. Recorded as a genuine follow-up rather than forced into "zero offenders" by silently narrowing what the guard actually checks
    - Also found and fixed while running the full suite: three hardcoded `getByLabelText("Template")` assertions in `run-form.test.tsx` (a `components/reports/` file already under the `messageText` discipline — its label rendered "Report profile" the moment the message value changed, and the test's own literal expectation was stale), plus the identity-rename simulation test's mocked fetch URLs
    - Suite: app 3287 passed / 0 failed / 0 skipped with a real Postgres (was 3281), typecheck clean except the one pre-existing error, lint unchanged at 13 problems
    - _Requirements: 1.1, 1.2, 1.3, 1.4, 1.5, 1.6, 1.7, 22.5, 22.6 (the literal-copy guard portion of 22.5/22.6 deferred, reason recorded above)_
