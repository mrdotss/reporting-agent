...
- [ ] 3. Phase 2 — sections
  ...
  - [ ] 3.1 `catalog/sections.v1.json` and its loader validation
  ...
  - [ ] 3.2 `schema_version` 3 in both halves, with the mirror and new corpus fixtures
  ...
  - [ ] 3.4 The `compile_document` branch
    - `agent/.../compile/__init__.py`: `if schema_version >= 3: specs = sections.expand_sections(...) else: specs = _block_specs(definition)`. Everything after — `_phase_one`, the prose deferral, `Document` assembly, `assert_ledger_matches_tree` — is untouched, because it consumes `Sequence[BlockSpec]`
    - Add keyword parameters `catalogue` and `authored_matches`, both defaulting to `None` so every existing caller and test compiles unchanged
    - Assert a v3 profile and a v2 definition that expand to the same block sequence produce **byte-identical** ledgers and anchor sets. That is the claim the whole design rests on, so it is a test and not a paragraph
