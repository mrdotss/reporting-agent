...
and not a guess from what it is called. `advisor` (task 6.4) is the fifth: Azure Advisor's
own recommendation feed, backing section 14's `recommendations` — a genuinely different
service from the four Recovery Services / Capacity / Resource Graph sources above it, not
...
"""The four gap types a **non-projectable** fact may name for its own absence
(Req 5.1-5.3, 16.7). `advisor_not_available` (task 6.4) is the fourth: Advisor answered
successfully and named no recommendation for a resource this run named — a data fact
about the resource, not a failure to ask.
